...
class BacktrackingSearch():
        ...
        if not self.ac3:
            ...
            for val in ordered_values:
                ...
                if deltaWeight > 0:
                    ...
                    self.arc_consistency_check(var, assignment)
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            fewestConsistentValues = float('inf')
            mostConstrainedVariable = self.csp.variables[1]
            for var in self.csp.variables:
                totalPossibleActions  = 0
                if var not in assignment:
                    for value in self.domains[var]:
                        if self.get_delta_weight(assignment, var, value) > 0:
                            totalPossibleActions +=1
                    if totalPossibleActions < fewestConsistentValues:
                        fewestConsistentValues = totalPossibleActions
                        mostConstrainedVariable = var
            return mostConstrainedVariable

    def arc_consistency_check(self, var, assignment):
        """
        ...
        # Hint: How to get variables neighboring variable |var|?
            for neighbor in self.csp.get_neighbor_vars(currentVar):
                #for each neighbor of the current variable we want to get all possible values
                #and see if they are still consistent, if they aren't we will remove them.
                localCopy = copy.deepcopy(self.domains[neighbor])
                initialSize = len(localCopy)
                for possibleValue in self.domains[neighbor]:
                    if self.csp.unaryFactors[neighbor] is not None and self.csp.unaryFactors[neighbor][possibleValue] == 0:
                        localCopy.remove(possibleValue)
                    ## If every assignment in current prevents a certain value in neighbor we remove
                    remove = True;
                    for currentValue in self.domains[currentVar]:
                        if self.csp.binaryFactors[currentVar][neighbor][currentValue][possibleValue] != 0:
                            remove = False;
                    if remove and possibleValue in localCopy:
                        localCopy.remove(possibleValue)
                self.domains[neighbor] = localCopy
                if len(localCopy) < initialSize:
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    """
    def constantDifference(aux, real):
        return aux[0] + real == aux[1]
    def postPreCheck(val1,val2):
        return val1[1] == val2[0]
    finalVar = name + "finalVar"
    finalDomain = [ j for j in range(0, maxSum+1)]
    csp.add_variable(finalVar,finalDomain)
    auxiliaryVars = [ name + "_" + str(i) for i in range(0, len(variables))]
    # the first 
    firstDomain = [(0,y) for y in csp.values[variables[0]]]
    csp.add_variable(auxiliaryVars[0], firstDomain)
    for j in range (1, len(auxiliaryVars)):
        #Get the domain of Ai-1
        oldDomain = csp.values[auxiliaryVars[j-1]]
        #Get all possible maximum sums it could be
        postProcessingOfPrev = [ post for pre, post in oldDomain]
        allPairs = []
        #make the new domain be all possible sums it could have been, + all values it could now be
        for possibleCurrentSum in postProcessingOfPrev:
            for num in csp.values[variables[j]]:
                allPairs.append((possibleCurrentSum,min(maxSum, possibleCurrentSum + num)))
        auxDomain = list(set(allPairs))
        #need to make sure that our post is the same as our pre plus the choice of real variable
        csp.add_variable(auxiliaryVars[j], auxDomain)
        csp.add_binary_factor(auxiliaryVars[j-1], variables[j-1], constantDifference)
        csp.add_binary_factor(auxiliaryVars[j-1],auxiliaryVars[j], postPreCheck)
    #must add for the last auxiliary variable
    csp.add_binary_factor(auxiliaryVars[len(auxiliaryVars)-1], variables[len(variables) -1], constantDifference)
    csp.add_binary_factor(auxiliaryVars[len(auxiliaryVars) -1], finalVar, lambda aux, final: aux[1] == final) 
    ...
    ## for each variable, make a new variable be the sum of it and one other, and then requeue it. 
    #range will be the sum of all the mins for each variable to the sum of all the max's
    # END_YOUR_CODE
...
class SchedulingCSPConstructor():
    ...
    def add_quarter_constraints(self, csp):
        ...
        # for each cid, make sure it is the the requests cid and the quarter 
        # is in one of the quarters in the requests
        for req in self.profile.requests:  
            for quarter in self.profile.quarters:
                csp.add_unary_factor((req, quarter), \
                lambda cid: cid is None or \
                cid in req.cids and \
                quarter in req.quarters or len(req.quarters) == 0 )           
    ...
    def add_unit_constraints(self, csp):
        ...
        # then use get sum to make sure they are consistent if they sum to all things from min to max
        # for all requests make the domain the min to the max
      
        def createDomain(mini, maxi):
            result = []
            if mini != 0: result.append(0)
        ...
        for quarter in self.profile.quarters: 
            ...
            for req in self.profile.requests:
                for cid in req.cids:
                    ...
                    domain = createDomain(course.minUnits, course.maxUnits +1)
                    newVar = (cid, quarter)
                    newVars.append(newVar)
                    #need to make sure that you can't take two classes from one requestS!
                    csp.add_variable(newVar, domain)
                    #new will be the number of units for a class and quarter
                    # old will be the cid for a certain request and quarter
                    csp.add_binary_factor((cid,quarter), (req,quarter), lambda numUnits, course: \
                        (course is None and numUnits is 0) or (course != cid and numUnits is 0)  or  (course == cid and numUnits is not 0 ))
                    #there are three cases, 1. Course is not taken and so units for request is 0
                    #                       2. A different course was chosen the request, so the number of units can be 0
                                            #3. The course chosen for request,quarter is the same as the cid and so we need the number of units to be positive
            overallConstraint = get_sum_variable(csp,  "unitConstraints" + quarter, newVars,self.profile.maxUnits)
            csp.add_unary_factor(overallConstraint, lambda v: v >= self.profile.minUnits) 
